...
const App: React.FC = () => {
  ...
  const generateSpecificInterval = useCallback((minNote: Note, maxNote: Note, intervalType: 'minor' | 'major', excludeBaseNote?: Note): IntervalPair | null => {
    ...
    const availableBaseNotes = excludeBaseNote 
      ? validBaseNotes.filter(note => note.midiNumber !== excludeBaseNote.midiNumber)
      : validBaseNotes;
    ...
    const baseNote = availableBaseNotes[Math.floor(Math.random() * availableBaseNotes.length)];
  ...
  const handleGuess = useCallback((guess: 1 | 2) => {
    ...
    const feedback = isCorrect 
      ? `Correct! The ${correctAnswer === 1 ? 'first' : 'second'} interval was larger.`
      : `Incorrect. The ${correctAnswer === 1 ? 'first' : 'second'} interval (${correctAnswer === 1 ? pair1Info : pair2Info}) was larger.`;
  ...
  const canGenerateIntervals = useCallback(() => {
    const { minNote, maxNote } = gameState;
    const minorSecondValidNotes = getValidBaseNotes(minNote, maxNote, INTERVALS[0]);
    const majorSecondValidNotes = getValidBaseNotes(minNote, maxNote, INTERVALS[1]);
    
    // We need at least 2 valid notes for each interval type to ensure different base notes
    return minorSecondValidNotes.length >= 1 && majorSecondValidNotes.length >= 1 && 
           (minorSecondValidNotes.length + majorSecondValidNotes.length >= 2);
  }, [gameState]);

  const isGameActive = gameState.currentPair1 && gameState.currentPair2 && !gameState.feedback;
    ...
    <div className="container">
      ...
      {!canGenerateIntervals() && (
        <div className="card" style={{ backgroundColor: '#fff3cd', border: '1px solid #ffeaa7' }}>
          <p>⚠️ The selected note range is too narrow to generate different intervals with different base notes. Please expand the range.</p>
        </div>
      )}
      ...
      <div className="card">
        ...
        <p><strong>Interval Types:</strong></p>
        <ul>
          <li><strong>Minor Second:</strong> 1 semitone (e.g., C to C#)</li>
          <li><strong>Major Second:</strong> 2 semitones (e.g., C to D)</li>
        </ul>
      </div>
    </div>
  );
